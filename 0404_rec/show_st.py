...
import joblib
from autoint import AutoIntPModel, predict_model
...
def load_data():
    ...
    field_dims = np.load(f'{data_path}/field_dims_mlp.npy')
    dropout= 0.4
    embed_dim= 16
    ...
    model = AutoIntPModel(field_dims, embed_dim, att_layer_num=3, att_head_num=2, att_res=True,
    ...
    model.load_weights(f'{model_path}/autoIntP_model_weights.h5')
    label_encoders = joblib.load(f'{data_path}/label_encoders_p.pkl')
